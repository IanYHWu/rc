...
class InferenceProblemState:
    ...
    This version does not use summarization - it conditions generation directly
    on the full reasoning trace from the previous step.
    """
    ...
    def __init__(
        ...
        # Current accumulated reasoning trace (full text from previous steps)
        self.curr_reasoning_trace = ""
        # Current step's reasoning output
        self.curr_reasoning = ""
        ...
        # Store accumulated traces at each step
        self.reasoning_trace_store = []
    ...
    def update_reasoning(self, rollouts: DataProto, response_string: str):
        ...
        # Set curr_reasoning_trace to the current reasoning output
        # (this will be used as context for the next step's generation)
        self.curr_reasoning_trace = self.curr_reasoning
        self.reasoning_trace_store.append(self.curr_reasoning_trace)

    def get_filled_reasoning_prompt(self) -> str:
        """Fill the reasoning prompt template with the full reasoning trace."""
        return self.reasoning_prompt_template.format(
            problem=self.problem,
            curr_reasoning_trace=self.curr_reasoning_trace,
        )
...
class ReplayBuffer:
    """Replay buffer for storing reasoning traces instead of summaries."""
    ...
    def load_from_file(self, data_path: str):
        ...
        for item in raw_data:
            ...
            if len(starting_steps) != len(reasoning_traces):
                raise ValueError(
                    f"Mismatch between number of reasoning_traces ({len(reasoning_traces)}) and "
    ...
    def sample_reasoning_traces_by_problem(self, problem: str, num_samples: int):
        if self.buffer is None:
            raise ValueError("Buffer is not loaded")
        if problem not in self.buffer:
            return []
        reasoning_traces = self.buffer[problem]["reasoning_traces"]
        starting_steps = self.buffer[problem]["starting_steps"]
        num_samples = min(num_samples, len(reasoning_traces))
        sampled_indices = np.random.choice(len(reasoning_traces), size=num_samples, replace=False)
        sampled_data = [(reasoning_traces[idx], starting_steps[idx]) for idx in sampled_indices]
        return sampled_data
    
    def purge_reasoning_traces_for_problems(self, problems: List[str]):
        if self.buffer is None:
            raise ValueError("Buffer is not loaded")
        for problem in problems:
            if problem in self.buffer:
                self.buffer[problem]["reasoning_traces"] = []
    ...
    def add_states_to_buffer(self, states: List[InferenceProblemState], purge_existing: bool = False):
        ...
        if purge_existing:
            ...
            self.purge_reasoning_traces_for_problems(list(problem_to_states.keys()))
            for state in states:
    ...
    def add_to_buffer(self, state: InferenceProblemState):
        ...
        reasoning_traces_to_add = []
        starting_steps_to_add = []
        for i, reasoning_trace in enumerate(state.reasoning_trace_store):
            trace_starting_step = state.starting_step + i + 1
            reasoning_traces_to_add.append(reasoning_trace)
            starting_steps_to_add.append(trace_starting_step)
        
        if problem in self.buffer:
            self.buffer[problem]["reasoning_traces"].extend(reasoning_traces_to_add)
            ...
            self.buffer[problem] = {
                ...
                "reasoning_traces": reasoning_traces_to_add
            }


class SummaryFreeRolloutGenerator:
    """
    Rollout generator that does not use summarization.
    Instead, it conditions generation directly on the full reasoning trace from the previous step.
    """
    ...
    def __init__(
        ...
        """
        Initialize the SummaryFreeRolloutGenerator.
        
        Note: This version does not take a summarization_prompt_template since
        summarization is not performed.
        """
        ...
        if self.thinking_train_samples_per_online_rollout == 0:
            raise ValueError("thinking_train_samples_per_online_rollout cannot be 0 in summary-free mode")
        ...
        if config.reasoning_cache.get("use_replay_buffer", False):
            ...
            self.buffer_sample_traces_per_problem = config.reasoning_cache.get("buffer_sample_summaries_per_problem", 1)
            ...
            print("Will use buffer to continue rollouts from existing reasoning traces when available.")
            if self.buffer_purge_existing_on_add:
                print("Note: Existing reasoning traces will be purged when adding new traces for the same problem.")
        else:
    ...
    def generate_rollouts(
        ...
        """
        Generate rollouts for reasoning cache without summarization.
        """
        ...
        print("Generating online rollouts (summary-free)...")
        if eval_mode:
        ...
        # Continue rollout steps - no summarization, just thinking steps
        ...
        # Prepare training states and prompts (only thinking, no summary)
        thinking_train_states = self.postprocess_online_rollout_states(online_rollout_states)
        ...
        thinking_reward_states = self.postprocess_train_states(thinking_train_states)
        ...
        if self.thinking_reward_rollout_steps > 0 and len(thinking_train_states) > 0:
            thinking_reward_states = self.rollout_step_thinking(
        ...
        # Postprocess train states and obtain DataProto for training (only thinking states)
        merged_data_proto = self.postprocess_completed_states(thinking_train_states)
    ...
    def create_snapshot_state(self, existing_state, thinking_ind):
        """Create a snapshot of the state at a given thinking step."""
        thinking_strings = existing_state.reasoning_string_store
        reasoning_traces = existing_state.reasoning_trace_store
        ...
        # Set curr_reasoning_trace to the reasoning from the step before thinking_ind
        # (this is the context that will be used for the next generation)
        if thinking_ind > 0 and len(reasoning_traces) > 0:
            snapshot_state.curr_reasoning_trace = reasoning_traces[thinking_ind - 1]
        else:
            snapshot_state.curr_reasoning_trace = ""
    ...
    def prepare_online_rollout_states(
        ...
        If replay buffer is enabled, checks each problem and initializes with sampled reasoning trace if available.
        """
        states = []
        num_from_buffer = 0
        num_from_scratch = 0
        ...
        for i in range(len(problems)):
            ...
            if self.use_replay_buffer and problem in self.replay_buffer:
                sampled_data = self.replay_buffer.sample_reasoning_traces_by_problem(
                    problem, self.buffer_sample_traces_per_problem
                )
                
                for reasoning_trace, trace_starting_step in sampled_data:
                    starting_step = trace_starting_step
                    ...
                    state.curr_reasoning_trace = reasoning_trace
                    states.append(state)
                    num_from_buffer += 1
            else:
                state = InferenceProblemState(
    ...
    def postprocess_online_rollout_states(self, online_rollout_states: List[InferenceProblemState]):
        ...
        Only returns thinking train states (no summary states in this version).
        """
        ...
        for state in online_rollout_states:
            ...
            if num_thinking_steps > 0:
                ...
                for ind in sampled_thinking_indices:
                    snapshot_state = self.create_snapshot_state(state, thinking_ind=ind)
    ...
    def postprocess_train_states(
        ...
        for state in thinking_train_states:
            thinking_reward_states.append(self.create_snapshot_state(state, thinking_ind=1))
        return thinking_reward_states
    ...
    def postprocess_completed_states(
        ...
        """
        Postprocess completed states to create DataProto for training.
        Only handles thinking train states (no summary states in this version).
        """
        reasoning_data_protos = [state.reasoning_rollout_store[-1] for state in thinking_train_states]

        prompt_ids = [f"R-{state.prompt_id}" for state in thinking_train_states]
        sample_ids = [state.sample_id for state in thinking_train_states]
        problem_ids = [state.problem_id for state in thinking_train_states]
        labels = [state.label for state in thinking_train_states]
        rewards = [state.final_reward for state in thinking_train_states]
        starting_step = [state.starting_step for state in thinking_train_states]

        merged_rollout_data_dict = self.merge_data(reasoning_data_protos)
        
        # Add dummy summary_indicator (all zeros - nothing is a summary in summary-free mode)
        batch_size = len(thinking_train_states)
        merged_rollout_data_dict["summary_indicator"] = torch.zeros(batch_size, dtype=torch.long)
