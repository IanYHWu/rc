...
class FSDPSFTTrainer(object):
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        new_tokens = self.config.model.get("additional_special_tokens", [])
        new_tokens = [str(t) for t in new_tokens]
        if len(new_tokens) > 0:
            special_tokens_dict = {
                "additional_special_tokens": new_tokens
            }
            self.tokenizer.add_special_tokens(special_tokens_dict, replace_additional_special_tokens=False)
            print_rank_0(f"Adding {len(new_tokens)} new tokens to the tokenizer: {new_tokens}")
            for token in new_tokens:
                print_rank_0(f"{token} now maps to ID {self.tokenizer.encode(token)}")
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(self.train_dataloader,
                ...
                # Save checkpoint
                if global_step >= self.total_training_steps or global_step % self.config.trainer.checkpoint_interval == 0:
